...
export function createMockAdminApi(getUser) {
  ...
  let deadLetters = [{ id: "outbox-dead-1", source: "outbox", eventType: "activity.registration.registered.v1", status: "dead_letter", attempts: 3, failureMessage: "Kafka broker unavailable", occurredAt: "2026-07-25T12:00:00", replayedAt: null }];
  ...
  return {
    ...
    async eventingOperations() { requireAdmin(); return { metrics: { pendingOutboxEvents: 0, retryingOutboxEvents: 0, deadLetterOutboxEvents: deadLetters.filter((item) => item.source === "outbox").length, deadLetterConsumerEvents: deadLetters.filter((item) => item.source === "consumer").length }, deadLetters: clone(deadLetters) }; },
    async replayDeadLetter(source, id) { requireAdmin(); const item = deadLetters.find((entry) => entry.source === source && entry.id === id); if (!item) throw new Error("死信事件不存在或已重放"); deadLetters = deadLetters.filter((entry) => entry !== item); addAudit(`重放${source === "outbox" ? " Outbox" : "消费者"}死信事件 ${id}`); },
