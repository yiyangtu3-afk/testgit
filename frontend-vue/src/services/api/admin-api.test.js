...
describe("admin API boundary", () => {
  ...
  it("keeps dead-letter replay HTTP failures instead of using Mock", async () => { const mockAdmin = { replayDeadLetter: vi.fn() }; const api = createAdminApi({ http: createHttpClient({ fetchImpl: vi.fn().mockResolvedValue(response(409)) }), mockAdmin }); await expect(api.replayDeadLetter("outbox", "dead-1")).rejects.toMatchObject({ name: ApiHttpError.name, status: 409 }); expect(mockAdmin.replayDeadLetter).not.toHaveBeenCalled(); });
});
