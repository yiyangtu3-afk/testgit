...
export function createChatRealtime({ token, onConversationEvent, onMode, WebSocketImpl = WebSocket }) {
  ...
  function connect() {
    ...
    socket.addEventListener("message", async (event) => { try { const payload = JSON.parse(event.data); if (["message.created", "message.withdrawn"].includes(payload.type) && payload.peerId) await onConversationEvent(payload.peerId); } catch { /* Ignore malformed realtime events. */ } });
