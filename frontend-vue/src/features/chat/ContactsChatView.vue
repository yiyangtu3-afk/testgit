...
const chat = useChatStore(); const session = useSessionStore(); const keyword = ref(""); const draft = ref(""); const attachments = ref([]);
let realtime;
onMounted(async () => { await chat.initialize(); if (session.mode === "api") { realtime = createChatRealtime({ token: () => session.token, onMode: () => {}, onConversationEvent: async (peerId) => { await chat.refreshLists(); if (peerId === chat.selectedId) await chat.loadMessages(peerId); } }); realtime.connect(); } });
  ...
  <section class="chat-workspace">
    ...
    <div class="chat-grid">
      <aside class="contact-pane">
        ...
        <section class="request-list"><p>好友申请</p><article v-for="request in chat.requests" :key="request.id"><span>{{ request.user?.name?.slice(0, 1) || "?" }}</span><div><strong>{{ request.user?.name }}</strong><small>{{ request.status === "pending" ? "待处理" : request.status }}</small></div><template v-if="request.direction === 'incoming' && request.status === 'pending'"><button @click="chat.resolveRequest(request.id, 'accept')">同意</button><button @click="chat.resolveRequest(request.id, 'reject')">拒绝</button></template></article></section>
