...
<template>
  <section class="chat-workspace">
    ...
    <div class="chat-grid">
      ...
      <section class="message-pane" v-if="chat.selectedFriend"><header><div><p class="eyebrow">PRIVATE CONVERSATION</p><h3>{{ chat.selectedFriend.name }}</h3></div><div class="conversation-actions"><span>{{ chat.selectedFriend.status || "在线" }}</span><button type="button" class="chat-contact-toggle" :aria-expanded="contactsOpen" @click="contactsOpen = !contactsOpen">{{ contactsOpen ? "收起联系人" : "联系人" }}</button></div></header><button v-if="chat.paging[chat.selectedId]?.hasMore" class="older" @click="loadOlderMessages">加载更早消息</button><div ref="messageStream" class="message-stream"><article v-for="message in chat.conversations[chat.selectedId] || []" :key="message.id" :class="{ mine: message.from === session.user?.id }"><p>{{ message.deleted ? "消息已撤回" : message.text }}</p><div v-if="!message.deleted && message.attachments?.length" class="attachment-row"><span v-for="file in message.attachments" :key="file.id">{{ file.name }} · {{ formatSize(file.size || 0) }}</span></div><small>{{ message.time }}</small><button v-if="message.from === session.user?.id && !message.deleted" @click="chat.withdraw(message.id)">撤回</button></article></div><form class="composer" @submit.prevent="send"><div v-if="attachments.length" class="pending-files"><span v-for="file in attachments" :key="file.id">{{ file.name }} <button type="button" @click="attachments = attachments.filter((item) => item.id !== file.id)">×</button></span></div><textarea v-model="draft" placeholder="输入消息…" rows="2"></textarea><footer><label class="file-button">添加附件<input type="file" multiple @change="addFiles" /></label><button class="send-button">发送</button></footer></form></section>
