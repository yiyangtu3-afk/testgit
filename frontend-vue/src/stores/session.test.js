...
describe("session store", () => {
  ...
  it("persists the account returned after registration", async () => {
    const api = {
      registerStudent: vi.fn().mockResolvedValue({ mode: "api", data: { token: "registered-jwt", user: { id: "u-new", name: "新同学" } } })
    };
    const session = createSessionStore({ api, storage: storage() })();

    await session.register("新同学", "13900000001", "123456");

    expect(api.registerStudent).toHaveBeenCalledWith("新同学", "13900000001", "123456");
