...
export function createAdminStore({ api, getUser = () => null, setMode = () => {} } = {}) {
  return () => {
    const metrics = ref({}), activities = ref([]), moderation = ref([]), audits = ref([]), notice = ref(""), loading = ref(false), report = ref(null), selectedModeration = ref([]), selectedAudits = ref([]);
    ...
    function clear() { metrics.value = {}; activities.value = []; moderation.value = []; audits.value = []; selectedModeration.value = []; selectedAudits.value = []; report.value = null; }
    async function load() { clear(); notice.value = ""; if (!isAdmin.value) return; loading.value = true; try { const [base, activityMetrics, pendingActivities, pendingModeration, auditEvents] = await Promise.all([run(() => api.metrics()), run(() => api.activityMetrics()), run(() => api.pendingActivities()), run(() => api.moderationItems()), run(() => api.auditEvents())]); metrics.value = { ...base, "活动报名": String(activityMetrics.registrationCount), "活动签到": String(activityMetrics.checkedInCount) }; activities.value = pendingActivities; moderation.value = pendingModeration; audits.value = auditEvents; } catch (error) { notice.value = error.message || "管理员数据暂时无法加载。"; } finally { loading.value = false; } }
    ...
    async function resolveModeration(id, decision) { try { await run(() => api.resolveModeration(id, decision)); notice.value = "内容审核已完成。"; await load(); } catch (error) { notice.value = error.message || "内容审核失败。"; } }
    ...
    return { metrics, activities, moderation, audits, notice, loading, report, selectedModeration, selectedAudits, isAdmin, load, toggle, reviewActivity, resolveModeration, deleteModeration, deleteAudits, generateReport };
  };
}
