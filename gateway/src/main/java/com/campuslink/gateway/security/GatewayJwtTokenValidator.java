...
/**
 * Verifies the signed and time-limited part of CampusLink's existing JWT at the edge.
 *
 * <p>The downstream API still validates its matching MySQL session and role for every request.
 * The original bearer token is forwarded unchanged; the gateway never trusts a forwarded identity
 * header.</p>
 */
@Component
public class GatewayJwtTokenValidator {
  ...
  @Autowired
  public GatewayJwtTokenValidator(@Value("${campuslink.security.jwt.secret}") String secret) {
    this(secret, Clock.systemUTC());
  }

  GatewayJwtTokenValidator(String secret, Clock clock) {
