...
当前前端版本为 `20260710-conversation-previews-v1`，本地验证地址为
`http://127.0.0.1:5179/?v=20260710-conversation-previews-v1`。当前基线保留
...
1. [x] 教师或社团负责人创建活动，管理员审核后发布后端垂直切片。
...
第一项当前只完成后端 API、权限、状态迁移和 MySQL 持久化。活动创建与审核
界面尚未实现，因此阶段二仍未形成前端可演示闭环。
