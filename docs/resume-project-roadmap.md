...
当前前端版本为 `20260710-chat-pagination-v1`，本地验证地址为
`http://127.0.0.1:5179/?v=20260710-chat-pagination-v1`。当前基线保留
...
6. 统一转义用户生成内容，消除直接写入 `innerHTML` 的 XSS 风险。
7. 为跨表业务流程补充服务层事务和数据库集成测试。
