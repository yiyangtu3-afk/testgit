...
当前前端版本为 `20260712-activity-filters-v1`，本地验证地址为
`http://127.0.0.1:5179/?v=20260712-activity-filters-v1`。当前基线保留
...
筛选或清除后会恢复当前报名状态。下一项进入持久化通知与 WebSocket 状态推送。
...
下一项实现持久化通知中心和活动状态 WebSocket 推送，先覆盖审核、报名、候补
和递补结果，再进入组织者名单、签到与导出。每个可演示小阶段都更新 README
和本路线的完成状态。
