...
当前前端版本为 `20260715-signed-jwt-logout-v1`，本地验证地址为
`http://127.0.0.1:5179/?v=20260715-signed-jwt-logout-v1`。当前基线保留
管理员审核工作台、普通用户审核反馈、普通文件附件卡片、窄窗口聊天
滚动修复，以及真实的动态可见范围过滤。当前功能稳定提交为
`98c2dad Add notification read and target actions`；详细约束见
...
3. [ ] 依次迁移联系人与聊天、动态、活动、通知和管理员模块。
4. [ ] 保持 Java API 不可达才回退 Mock、JWT 身份边界、实时通知和 XSS 转义语义。
5. [ ] 新旧版本逐项验收后，经明确确认才切换默认入口。
...
认证和应用壳切片已经完成：`frontend-vue/` 包含 Vite、Vue Router、Pinia、认证、
API/Mock 边界、导航与统一状态提示，旧静态入口保持不变。下一项是联系人、好友申请
和聊天。每个可演示小阶段都更新 README 和本路线的完成状态。
