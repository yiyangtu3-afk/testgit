...
应用壳、联系人与聊天、动态、活动和通知六个 Vue 切片。通知使用相对代理 API、持久化
摘要、受限目标解析和已认证 WebSocket 事件，且 HTTP `4xx`、`5xx` 不回退 Mock。
旧静态入口保持不变；下一项是管理员模块。每个可演示小阶段都更新 README 和本路线的
完成状态。
