...
旧版静态基线保留管理员审核工作台和普通用户审核状态反馈，不保留后来
试做的聊天图片消息、图片预览、图片缓存、搜索好友直接聊天、动态页强制
刷新、Java API 空态文案、以及 portal 风格登录页。用户已明确授权 Vue
默认入口提供新的受认证图片消息能力；它不改动旧版静态入口或旧版回归基线。
...
内容审核现在要求管理员在拒绝时填写审核意见。审核记录保留审核人、审核时间和意见，
审计事件也包含相同信息。Vue 内容审核工作台只请求和显示 `pending` 内容；已完成决定
不再留在该功能框中，因而旧版默认待审队列行为不变。

管理员还可为单条待审核内容生成审核辅助建议。当前实现使用本地可解释规则，不调用外部
模型或传输内容；它只返回风险等级、命中信号和建议意见。建议不会自动填写审核意见、改变
内容状态或写入审核、审计记录，最终决定仍必须由管理员操作产生。
完整边界、目录设计、验证方式和下一切片见
[`vue-migration-handoff.md`](vue-migration-handoff.md)。

## 附件 UUID 兼容性修复（2026-07-21）

Vue 聊天选择图片或普通文件时，浏览器会生成 36 位 UUID 作为附件标识。
后端附件表原先只为该标识预留 32 位，MySQL 因字段过长返回 `500`。
现在新建表和已有本地表都将 `message_attachments.id` 保持为
`varchar(64)`；迁移会先读取 `information_schema`，只在字段长度不足时
扩大列，不删除、重置或重种任何聊天历史。

容器化 MySQL 集成测试会写入真实 36 位浏览器 UUID，并在事务结束后回滚
测试数据。后续的 Vue 图片消息实现继续使用该 64 位标识。

## Vue 图片消息（2026-07-21）

用户明确授权 Vue 聊天恢复图片消息。Vue 可以发送 PNG、JPEG、WebP 和 GIF，
单张图片最多 5 MB。发送时图片字节与消息附件一同持久化到 MySQL；普通的
旧附件记录不会被修改或重写。

历史消息和实时消息都会显示缩略图。点击缩略图会打开大图预览层。图片读取
通过 `GET /api/conversations/{peerId}/attachments/{attachmentId}`，必须携带当前
JWT，并再次验证当前用户与会话对方仍是好友；已撤回消息和未保存图片字节的
旧元数据附件不会返回内容。Vue 对这类旧附件保留普通文件卡片，不显示加载错误。

## Vue 活动签到凭证（2026-07-23）

Vue 活动页新增受认证签到凭证。已报名学生可以展示当前不透明码，组织者在
**我的活动运营** 输入后由服务端校验 JWT、活动归属和报名状态，再写入既有
`checked_in` 事务与事件历史。数据库只保存 SHA-256 凭证摘要；再次展示会轮换旧码。
此功能只位于 `frontend-vue/`，没有修改旧静态入口或 `frontend/js/`。
...
- **待审核内容** 列表展示标题、提交人、提交时间、内容类型、当前状态
  和操作。
- 管理员可执行 **同意**、**拒绝**、**查看**。
- **待审核内容** 支持多选后的 **删除所选**；审计记录还支持 **全选**、取消全选后
  再执行 **删除所选**。
- 未勾选时 **删除所选** 禁用，勾选后启用。
...
- 登录页保持黑色右侧信息面板版本：**简洁的校园沟通空间**，右侧展示
  **身份**、**状态**、**模式** 三个信息块。
- 旧版静态聊天附件保持普通文件卡片展示；Vue 聊天支持受认证图片缩略图和
  点击大图预览。
...
不要把下面这些后续试做内容重新接入旧版静态基线。它们与经过用户明确授权、
仅在 `frontend-vue/` 实现的当前图片消息无关。
...
当前仓库里可能还存在 `frontend/js/chat/attachments.js`，它是旧版图片消息
试做期间创建的兼容空模块。当前主代码不引用它。不要将 Vue 图片消息实现
回迁到这个旧版模块。
...
2. 显式加载 Byte Buddy agent 的完整 Maven 测试通过，`151` 个测试无失败、
...
6. 2026 年 7 月 21 日运行 `node script/run_live_equivalence_check.mjs`：旧版静态入口
   返回 `200`；Vue 代理和旧版直连 API 对学生动态、活动、两类通知、聊天未读与历史、
   活动运营与名单、管理员指标、审核历史、审计记录和审核辅助的 JSON 响应完全一致。
   无效动态通知目标和无效聊天撤回请求均在两条路径返回相同的 `400`。脚本只创建并注销
   本次演示会话，不写入活动、消息、审核或通知业务数据。
...
审核辅助首个切片已完成：本地规则只提供风险提示或建议理由，管理员仍需要执行最终审核
动作。若未来接入外部模型，必须经明确授权，并继续保持人工审核边界和 API 错误不回退
Mock 的规则。
...
- 旧版静态聊天附件保持普通文件卡片展示；Vue 图片消息使用独立的受认证
  存取和预览实现。
- 登录页是黑色右侧信息面板版本，文案为“简洁的校园沟通空间”。

明确不要把这些后续试做内容重新接入旧版静态基线：
