...
好友申请通知和评论通知切片也已完成。下一项继续社交通知实时推送，不要重做
已完成的聊天、动态、内容审核和活动报名链路。
...
静态前端版本为 `20260715-comment-notifications-v1`，本地地址为：
...
阶段二和阶段三点赞、好友申请和评论通知切片已经完成。下一项把社交通知接入
现有 WebSocket 通道，实时刷新收件人的未读数。继续保持 bearer token 身份
