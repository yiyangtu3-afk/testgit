...
静态前端版本为 `20260711-activity-review-layout-v2`，本地地址为：

```text
http://127.0.0.1:5179/?v=20260711-activity-review-layout-v2
```
...
13. 前端缓存版本升级为 `20260711-activity-review-layout-v2`，确保浏览器重新
...
学生报名、取消报名与候补队列的设计已整理到
[`activity-registration-design.md`](activity-registration-design.md)。下一项实现
独立活动领域的报名持久化层：先增加当前报名和追加式报名事件表，并补齐可回滚
的 MyBatis 集成测试；不要在这一步加入前端按钮、通知、名单或签到。
