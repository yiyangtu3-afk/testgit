...
好友申请通知、评论通知和社交通知实时推送切片也已完成。下一项补齐通知操作，
...
静态前端版本为 `20260715-social-realtime-v1`，本地地址为：
...
阶段二和阶段三点赞、好友申请、评论通知与实时推送切片已经完成。下一项补齐
单条通知已读与可追溯的目标跳转。继续保持 bearer token 身份边界、真实 API
错误不回退 Mock、跨表写入事务和可回滚 MyBatis 集成测试。
